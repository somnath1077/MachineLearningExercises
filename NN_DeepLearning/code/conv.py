...
from NN_DeepLearning.code import network3
from NN_DeepLearning.code.network3 import ConvPoolLayer, FullyConnectedLayer, SoftmaxLayer
from NN_DeepLearning.code.network3 import sigmoid, ReLU, Network
...
# basic l2 reg didn't help much.  The reason (I believe) is
...
def regularized_dbl_conv():
    for lmbda in [0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0, 100.0]:
        for j in range(3):
            print("Conv + Conv + FC num %s, with reg %s" % (j, lmbda))
            net = Network([
                ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28),
                              filter_shape=(20, 1, 5, 5),
                              poolsize=(2, 2)),
...
def dbl_conv_relu():
    for lmbda in [0.0, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0, 100.0]:
        for j in range(3):
            print("Conv + Conv + FC num %s, relu, with reg %s" % (j, lmbda))
            net = Network([
                ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28),
                              filter_shape=(20, 1, 5, 5),
                              poolsize=(2, 2),
