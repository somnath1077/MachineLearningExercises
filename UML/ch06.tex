...
extensions in $\bigcup_{i = 1}^r \hypclass_i$. But Sauer's Lemma, for any 
...
This is at most $k^d$ since $d \geq 3$. Hence the total number of extensions 
possible in the union on a set of size $k$ is at most $r \cdot k^d$. Hence we must
have $2^k \leq r \cdot k^d$, which implies that 
$k \leq d \cdot \log_2 k + \log_2 r < d \cdot \log_e k + \log_e r $.
