...
$\vcdim(\pos(\mathcal{F} + g)) \geq \vcdim (\mathcal{F})$. Let $C \subseteq \Rone^n$ be a 
...
\[
\alpha_1 := \frac{\max_{x \in C'} |g(x)|}{\min_{x \in C'} h_1(x)}, \qquad 
\alpha_2 := \frac{{\max_{x \in C \setminus C'} |g(x)|}}{\min_{x \in C \setminus C'} h_2(x)}
\]
Then $\alpha_1 h_1(x) - \alpha_2 h_2(x) + g(x) > 0$ iff $x \in C'$, showing that
$\pos (\mathcal{F} + g)$ also shatters $C$. Hence 
$\vcdim(\pos(\mathcal{F} + g)) \geq \vcdim (\mathcal{F})$.

We next show that $\vcdim(\pos(\mathcal{F} + g)) \leq \vcdim (\mathcal{F})$. Now let 
...
$\vcdim(\pos(\mathcal{F} + g)) \leq \vcdim (\mathcal{F})$. 
