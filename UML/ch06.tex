...
\begin{enumerate}
    ...
    \item Let $\basisvec_1, \ldots, \basisvec_d$ be the standard basis vectors of
        ...
        and each such function can actually be represented by a boolean conjunction
        on $d$ variables. This means that no matter which function 
        $(b_1, \ldots, b_d)$ we select, there exists a function in $\conj{d}$ 
        whose restriction to the basis vectors is precisely $(b_1, \ldots, b_d)$. 
        In order to be able to discover that function, we can use the 
        algorithm that implements the ERM 
        rule (as in Exercise 3.4) using the basis vectors as the sample set. 
    
        This algorithm starts with the conjunction: $x_1 \wedge \bar{x}_1 \wedge 
        \cdots \wedge x_{d} \wedge \bar{x}_d$. It then runs through the basis 
        vectors that have been mapped to $1$ by $(b_1, \ldots, b_d)$ and drops 
        all literals from the conjunction which would otherwise cause it to 
        evaluate to $0$. The conjunction output by the algorithm was proved to 
        have an in-sample loss of $0$, which means that it matches the 
        function $(b_1, \ldots, b_d)$. Thus the set of basis vectors is 
        shattered by $\conj{d}$ and hence $\vcdim (\conj{d}) \geq d$.
