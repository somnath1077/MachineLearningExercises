...
\subsection*{6.12.2}
 ...
 We wish to show that $\vcdim (\pos (\mathcal{F})) \leq \dim (\mathcal{F})$, where 
...
\]
Hence every function in $\pos (\mathcal{F} )$ can be represented by a homogenous linear 
halfspace in $\Rone^d$. Since the VC-dimension of the set of \emph{all} homogeneous linear
halfspaces in $\Rone^d$ is $d$, we have: 
$\vcdim (\pos (\mathcal{F})) \leq d$.
