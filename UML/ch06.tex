...
is a distribution, either $s_0 \neq 0$ or $s_1 \neq 1$. 

Given a sample $S$, let $t_0 = \max \{t \colon (t, 0) \in S\}$ and $t_1 = \min
\{t \colon (t, 1) \in S\}$. The ERM algorithm outputs $h_p$, where $p \in (t_0,
t_1)$.  In particular, if the sample presented to the ERM algorithm contained a
point from $[s_0, s_1]$, the hypothesis $h_p$ returned by the ERM algorithm
will incur a loss of $L_{\dist}(h_p) \leq \epsilon$.
...
\begin{align*}
    \Prtwo{S \sim \dist^m}{L_{\dist} (\ERM(S)) > \epsilon} 
     & = \Prtwo{S \sim \dist^m}{S \colon S|_x \cup [s_0, s_1] = \emptyset} \\
     & \leq  \Prtwo{S \sim \dist^m}{S \colon S|_x \cup [s_0, s] = \emptyset} + 
             \Prtwo{S \sim \dist^m}{S \colon S|_x \cup [s, s_1] = \emptyset} \\      
